...
import { findCurrentLineIndex } from '../services/lrcParser'
...
export function useSyncedLyrics(options: UseSyncedLyricsOptions = {}) {
  ...
  const startPlayback = useCallback((startTimeMs: number = 0) => {
    ...
    // Cancel any existing animation frame before starting a new one to prevent leaks
  ...
  // Intentionally omit stopPlayback/startPlayback/setCurrentLineIndex from deps —
  // they are stable callbacks but including them would re-trigger on every render.
  // syncedLines identity change is the correct and only trigger here.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [syncedLines, isTimed, enabled])
