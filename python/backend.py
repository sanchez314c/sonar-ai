...
def get_window_title(service: StreamingService) -> str:
    ...
    if sys.platform == "win32":
        ...
        if not window_name:
            try:
                windows = subprocess.check_output(["xwininfo", "-tree", "-root"]).decode("utf-8")
...
def open_spotify(service: StreamingService) -> bool:
    if sys.platform == "win32":
        if not get_window_title(service):
            exe_path = service.get_windows_exe_path()
        ...
        if not get_window_title(service):
            cmd = service.get_linux_open_command()
            if not shutil.which(cmd):
                return False
            subprocess.Popen([cmd])
    elif sys.platform == "darwin":
        if not get_window_title(service):
            app_name = service.get_apple_open_command()
...
def main():
    if os.name == "nt":
        subprocess.run(["chcp", "65001"], shell=False)
