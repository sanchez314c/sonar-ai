...
"""
CURRENT_SERVICE = -1
_SERVICE_LOCK = threading.Lock()  # Guard CURRENT_SERVICE for concurrent IPC calls
SECONDS_IN_WEEK = 604800
LyricsMetadata = namedtuple(
    "LyricsMetadata", ["lyrics", "url", "service_name", "timed"]
)
...
def cache_lyrics(func):
    ...
    def wrapper(*args, **kwargs):
        song = args[0]
        _sync = kwargs.get("sync", False)  # passed through to wrapped func via **kwargs
...
def load_lyrics(song: Song, **kwargs):
    ...
    with _SERVICE_LOCK:
        ...
        current_not_synced_service = (
            -1 if current_not_synced_service < -1 else current_not_synced_service
        )
        if (
            sync
            and not lyrics
            or not sync
            or CURRENT_SERVICE > (len(s.SERVICES_LIST1) - 1)
        ):
            for i in range(current_not_synced_service + 1, len(s.SERVICES_LIST2)):
                result = s.SERVICES_LIST2[i](song)
                if result:
                    # _local returns 4 values (lyrics, url, service_name, timed);
                    # unsynced scrapers return 3 (lyrics, url, service_name).
                    if len(result) == 4:
                        lyrics, url, service_name, timed = result
                    else:
...
def get_window_title(service: StreamingService) -> str:
    ...
    if sys.platform == "win32":
        ...
        try:
            r = applescript.tell.app(
                service.get_apple_open_command(), service.get_apple_script()
            )
            window_name = r.out
        ...
        try:
            ...
            spotify_dbus = session.get_object(
                "org.mpris.MediaPlayer2.%s" % service.get_linux_session_object_name(),
        ...
        if not window_name:
            try:
                ...
                svc_name = service.get_linux_open_command().lower()
                for line in windows.splitlines():
                    if '("' + svc_name + '" "' + svc_name + '")' in line.lower():
                        if " - " in line:
